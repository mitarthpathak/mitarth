...
// and not embeddings at this size). MiniSearch indexes title and text, with
// the title boosted and light fuzzy + prefix matching. The index is built
// once per server instance; a query takes well under a millisecond.
...
export function retrieve(question, k = TOP_K) {
  ...
  const results = hits.map((h) => ({ ...byId.get(h.id), score: Number(h.score.toFixed(3)) }));
  if (!results.some((r) => r.id === PROFILE_SUMMARY_ID)) {
    results.push({ ...byId.get(PROFILE_SUMMARY_ID), score: 0 });
  }
  return results;
}
