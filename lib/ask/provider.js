...
// Picks the model from environment variables. Server-only: the keys are read
// here and nowhere else, and only /api/ask (through handle.js) calls it.
//
...
export async function languageModel({ provider, model }) {
  switch (provider) {
    ...
    case "mock":
      // A static NODE_ENV check, so production builds drop the mock entirely.
      if (process.env.NODE_ENV !== "production") return (await import("./mock-model.js")).mockModel();
      throw new Error("The mock model is development-only");
    default:
